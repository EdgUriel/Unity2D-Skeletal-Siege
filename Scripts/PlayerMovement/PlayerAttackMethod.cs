...
namespace EthanTheHero
{
	public class PlayerAttackMethod : MonoBehaviour
	{
		...
		[SerializeField] private float basicAttack01Damage = 10;
		[SerializeField] private float basicAttack02Damage = 10;
		[SerializeField] private float basicAttack03Damage = 10;

		[Header("Mover jugador al atacar")]
		...
		private void BasicAttackCombo()
		{
			...
			if (myAnim.GetCurrentAnimatorStateInfo(0).IsName("Attack01"))
			{
				...
				foreach (Collider2D collisionador in objetos)
				{
					...
					{
						collisionador.GetComponent<Health>().TakeDamage(1);
					}
				}
			}
		...
		private void BasicAttackMethod() //Move player when attacking
		{
